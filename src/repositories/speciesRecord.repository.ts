...
export const speciesRecordRepository = {
  findByStatus(status: RecordStatus) {
    return prisma.speciesRecord.findMany({
      where: { status },
      include: { species: true, period: true, inputter: true, verifier: true, index: true },
      orderBy: { createdAt: 'desc' },
    });
  },

  // Riwayat milik SATU pengguna (Petugas Lapangan melihat submission-nya sendiri
  // lintas semua status) -- boleh difilter status tertentu jika diperlukan.
  findByOwner(userId: string, status?: RecordStatus) {
    return prisma.speciesRecord.findMany({
      where: { inputBy: userId, ...(status ? { status } : {}) },
      include: { species: true, period: true, verifier: true, index: true },
      orderBy: { createdAt: 'desc' },
    });
  },
  ...
  updateStatus(
    id: string,
    status: RecordStatus,
    extra: { verifiedBy?: string | null; catatanRevisi?: string | null } = {}
  ) {
    return prisma.speciesRecord.update({ where: { id }, data: { status, ...extra } });
  },

  delete(id: string) {
    return prisma.speciesRecord.delete({ where: { id } });
  },
