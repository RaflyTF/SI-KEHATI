...
export default function LaporanPage() {
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <h1 className="text-lg md:text-xl font-semibold">Laporan</h1>
      {error && <p className="text-sm text-danger">{error}</p>}

      <Card padding="p-4 md:p-5">
        <h2 className="text-sm font-medium mb-4">Laporan Data Flora & Fauna</h2>
        <div className="flex flex-col sm:flex-row gap-2">
          <Button className="w-full sm:w-auto" disabled={!!loading} onClick={() => exportSpeciesRecords('pdf')}>
            {loading === 'records-pdf' ? 'Memproses...' : 'Ekspor PDF'}
          </Button>
          <Button
            variant="secondary"
            ...
            disabled={!!loading}
            onClick={() => exportSpeciesRecords('excel')}
          >
            {loading === 'records-excel' ? 'Memproses...' : 'Ekspor Excel'}
          </Button>
        </div>
      ...
      <Card padding="p-4 md:p-5">
        <h2 className="text-sm font-medium mb-4">Laporan Program Konservasi</h2>
        <div className="flex flex-col sm:flex-row gap-2">
          <Button className="w-full sm:w-auto" disabled={!!loading} onClick={() => exportPrograms('pdf')}>
            {loading === 'programs-pdf' ? 'Memproses...' : 'Ekspor PDF'}
          </Button>
          <Button
            variant="secondary"
